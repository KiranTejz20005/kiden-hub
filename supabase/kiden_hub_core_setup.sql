-- Kiden Hub Core Setup
-- Minimal schema for the tables the current app actively uses.
...
create policy "Users can view accessible workspaces" on public.workspaces for select using (
...
create policy "Users can create workspace members" on public.workspace_members for insert with check (
  auth.uid() is not null and (
    auth.uid() = user_id
    or public.user_has_workspace_access(workspace_id)
    or public.user_owns_workspace(workspace_id)
...
create policy "Users can update workspace members" on public.workspace_members for update using (
  auth.uid() is not null and (
    auth.uid() = user_id
    or email = auth.email()
    or public.user_has_workspace_access(workspace_id)
...
create policy "Users can delete workspace members" on public.workspace_members for delete using (
  auth.uid() is not null and (
    auth.uid() = user_id
    or email = auth.email()
    or public.user_has_workspace_access(workspace_id)
...
create policy "Users can manage collections" on public.collections for all using (
  auth.uid() is not null and (
    auth.uid() = user_id
    or (workspace_id is not null and public.user_has_workspace_access(workspace_id))
  )
...
create policy "Users can manage own files" on public.files for all using (
  auth.uid() is not null and (
    auth.uid() = user_id
    or (workspace_id is not null and public.user_has_workspace_access(workspace_id))
  )
...
create policy "Users can manage conversations" on public.conversations for all using (
  auth.uid() is not null and (
    auth.uid() = user_id
    or (workspace_id is not null and public.user_has_workspace_access(workspace_id))
  )
...
create policy "Users can manage messages" on public.messages for all using (
  auth.uid() is not null and public.user_has_conversation_access(conversation_id)
) with check (
  auth.uid() is not null and public.user_has_conversation_access(conversation_id)
);

create policy "Users can manage research boards" on public.research_boards for all using (
  auth.uid() is not null and (
    auth.uid() = user_id
    or (workspace_id is not null and public.user_has_workspace_access(workspace_id))
  )
