...
const MyBoards = ({ resetCounter = 0 }: { resetCounter?: number }) => {
  const { user } = useAuth();
  ...
  const fetchBoards = useCallback(async () => {
    ...
    try {
      ...
      const normalizedBoards = ((data ?? []) as any[]).map((board) => ({
        ...board,
        item_count: typeof board.item_count === 'number' ? board.item_count : 0,
      })) as Board[];

      setBoards(normalizedBoards);
      setSelectedBoard((current) => {
        if (!normalizedBoards.length) return null;
        if (!current) return normalizedBoards[0];
        return normalizedBoards.find((board) => board.id === current.id) ?? normalizedBoards[0];
      });
    } catch (error) {
  ...
  return (
    <div className="h-full flex min-h-0 overflow-hidden bg-background text-foreground">
      <aside className="w-64 shrink-0 border-r border-white/5 bg-[#090909] flex flex-col min-h-0">
        <ScrollArea className="flex-1 px-3 py-6">
          <div className="space-y-6">
            {/* Boards List */}
            <div className="space-y-1">
              <div className="flex items-center justify-between px-3 mb-2">
                <span className="text-[10px] font-bold uppercase tracking-widest text-white/30">My Space</span>
                <button onClick={openCreateModal} className="p-1 rounded-md hover:bg-white/10 text-white/30 hover:text-white transition-all">
                  ...
                  filteredBoards.map((board) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all text-[13px] font-medium",
                          isActive ? "bg-white/5 text-white" : "text-white/50 hover:text-white hover:bg-white/5"
                        )}
                      >
                        <span className="text-sm shrink-0 opacity-70">{board.emoji || '📁'}</span>
            ...
            <div className="flex-1 flex items-center justify-center p-8">
              <div className="w-full max-w-xl bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-10 md:p-14 shadow-2xl relative overflow-hidden group/board">
                ...
                <div className="relative z-10">
                  ...
                  <div className="space-y-1">
                    {[
                      { icon: LinkIcon, label: 'Paste a link', shortcut: 'Ctrl V', type: 'shortcut' },
                      { icon: FileText, label: 'Start a document', shortcut: 'D', type: 'shortcut' },
                      { icon: MessageSquare, label: 'Jot quick ideas or drafts', shortcut: 'C', type: 'shortcut' },
                      { icon: Upload, label: 'Drop images, PDFs, or files', shortcut: 'DRAG', type: 'action' },
                      { icon: TrendingUp, label: 'Save top performing content', shortcut: 'DISCOVER', type: 'action' },
                      { icon: Sparkles, label: 'Save an AI chat response', shortcut: 'CHAT', type: 'action' }
                    ].map((item) => (
                      <button 
                        key={item.label}
